...
本项目是一个基于 **Gate.io U本位永续合约 API** 的 MA10（10周期移动平均线）趋势监控系统。系统支持 **Web 可视化面板** 和 **命令行脚本** 两种使用方式，可实时监控多个加密货币和美股代币的日K、60分钟、15分钟三个时间周期的 MA10 趋势方向（连续上涨 / 连续下跌 / 震荡）。
...
| **统计栏** | 日K上涨数 / 日K下跌数 / 60分钟上涨数 / 60分钟下跌数 / 15分钟上涨数 / 15分钟下跌数 |
...
- [ ] **趋势转折告警**：当 MA10 从"连续下跌"转为"连续上涨"（或相反）时推送通知
- [ ] **前端标的管理**：页面直接增删监控标的，无需修改 JSON 文件

### P2 - 部署与通知
- [ ] **云服务器部署**：部署到香港/新加坡 VPS，实现 24h 随时访问
- [ ] **推送通道**：微信/钉钉/企业微信/Telegram Bot
